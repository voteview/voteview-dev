...
class MigrationEngine(object):
    ...
    def get_migration_files(self):
        ...
        migrations = (
            re.match(MigrationFile.PATTERN, filename)
    ...
    def get_unregistered_migrations(self):
        ...
        return [
            m
            for m in self.get_migration_files()
    ...
    def check_directory(self):
        ...
        if not exists:
            logger.error(
                "No migrations directory found. Check your path or create a migration first."
            )
    ...
    def get_migrations_to_up(self, migration_id=None):
        ...
        if migration_id:
            try:
                ...
                logger.error("Migration is not in unregistered list: %s" % migration_id)
                self.show_status()
                return []
        else:
    ...
    def get_migrations_to_down(self, migration_id):
        ...
        return list(
            reversed(
                [m for m in migrations if migration.id <= m.id <= last_migration_id]
            )
        )
